...
function ModuleTooltip({ modules, children }: { modules: ModuleInfo[]; children: React.ReactNode }) {
        ...
        <div className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-3 z-[9999]">
          <div className="bg-gray-900 text-white rounded-xl shadow-2xl border border-gray-700 min-w-80 max-w-2xl">
            ...
            <div className="px-4 py-3 border-b border-gray-700">
              <div className="font-semibold text-sm text-gray-100">Modules ({modules.length})</div>
            </div>
                ...
                {modules.map((module) => (
                  ...
                  >
                    <div className="flex items-start justify-between mb-2">
                      <span className="font-bold text-blue-300 text-sm">{module.moduleId}</span>
                      <span className="text-xs text-gray-400 bg-gray-700 px-2 py-1 rounded-full">
                        {module.moduleCredit} MC
                      </span>
                    </div>
                    <div className="text-gray-200 text-sm leading-relaxed mb-2">{module.title}</div>
                    <div className="text-xs text-gray-400">{module.faculty}</div>
                  </div>
                ))}
              </div>
            </div>
...
export default function CollaboratorsSection({
  ...
  return (
    <div className="p-4 border-t bg-white">
      ...
      <p className="text-sm text-gray-600 mb-4">Toggle visibility of each person's schedule</p>
        ...
        {collaborators.map((collaborator) => (
          <div key={collaborator.userId} className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              <span className="font-medium">{collaborator.userId === "You" ? "You" : collaborator.userId}</span>
              <ModuleTooltip modules={collaborator.modules}>
                <span className="text-sm text-gray-600 cursor-help hover:text-blue-600 hover:underline transition-colors font-medium">
                  {collaborator.moduleCount} modules
                </span>
              </ModuleTooltip>
            </div>
          </div>
